    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if not isinstance(expr1, Expression):
        if isinstance(expr2, Expression):
            for term in terms_2: factors.append(Product([term_1,term]))
    if not isinstance(expr2, Expression):
        if isinstance(expr1, Expression):
            for term in terms_1: factors.append(Product([term,term_2]))
    if not isinstance(expr2, Expression) & isinstance(expr1, Expression): factors.append(Product([term_1,term_2]))
    ---------------------------1
    if isinstance(expr1, Sum) & isinstance(expr2, Sum):
        for term1 in terms_1:
            for term2 in terms_2:
                factors.append(Product([term1,term2]))
    --------------------------2
    if isinstance(expr1, Sum) & isinstance(expr2, Product):
        for term in terms_1:
            factors.append(Product([term,term_2]))
    --------------------------3
    if isinstance(expr1, Product) & isinstance(expr2, Sum):
        for term in terms_2:
            factors.append(Product([term_1,term]))
    --------------------------4
    if isinstance(expr1, Product) & isinstance(expr2, Product):
        ...
        for term in terms_2:
            factors.append(term)
    #--------------------------
    return Product([factors]).flatten()
